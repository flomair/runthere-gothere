...
- **race stages** in shared journeys: turn any stretch of the route into a race between two or more members with a fixed time window and a prize. Each runner's baseline (average weekly distance of the 4 weeks before the start, at least 5 km) is locked when the stage begins, and progress is shown as effort: distance in the window compared with that baseline. Only recorded Strava activities count (no manual entries). Stage wins are tallied, and the owner can promise a bonus prize at the final destination for the most wins
- **surprise bucket** in shared journeys: everyone drops hidden prizes (small, medium or rare, optionally anonymous) into the group's bucket. Draws are earned by passing the "?" pins on the map, reaching cities, halfway and the finish, winning race stages, and random lucky weeks. The server picks the prize (never your own), with better odds for rare prizes after bigger achievements (a hard-won stage beats a pin), and logs every draw with its odds. A reveal animation shows what you got; then mark it delivered, with a photo if you like. The group is notified when the bucket runs low
- **challenges & gifts up front**: the home screen and each journey open with colourful tiles for what's waiting (challenges to answer, rewards to claim, gifts owed, mystery draws, running quests and stages, the next reward on the route). A gift button in the header shows how many need you, each journey has a *Gifts* tab, and *Challenges & gifts* collects everything in one place
