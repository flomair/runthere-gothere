            ...
            {frames.map((f, i) => (
              <Box key={f.photo.id} onClick={() => setIdx(i)} sx={{ flex: 1, height: 4, borderRadius: 2, bgcolor: 'rgb(255 255 255 / 30%)', overflow: 'hidden', cursor: 'pointer' }}>
                <Box
                  ...
                  sx={{ height: '100%', bgcolor: '#5B5BF0' }}
                />
              </Box>
            ))}
          </Stack>
          <Stack direction="row" sx={{ alignItems: 'center', gap: 1.5, flexWrap: 'wrap' }}>
            <IconButton aria-label={playing ? t('Pause') : t('Play')} onClick={() => (idx >= frames.length - 1 ? (setIdx(0), setPlaying(true)) : setPlaying((p) => !p))} disabled={frames.length < 2} sx={{ color: '#fff', bgcolor: '#5B5BF0', '&:hover': { bgcolor: '#5B5BF0', filter: 'brightness(1.08)' } }}>
            ...
            <ToggleButtonGroup
              ...
              sx={{ '& .MuiToggleButton-root': { color: 'rgb(255 255 255 / 75%)', borderColor: 'rgb(255 255 255 / 25%)', py: 0.25 }, '& .Mui-selected': { color: '#fff !important', bgcolor: 'rgb(91 91 240 / 55%) !important' } }}
            >
